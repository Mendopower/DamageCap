"""BLIP-3 instruction fine-tuning with Phi-3 LoRA and a trainable vision tokenizer."""
import argparse
...
def apply_phi3_lora(model, args):
    model.lang_model = get_peft_model(
        model.lang_model,
        LoraConfig(
            r=args.lora_r,
    ...
    if args.gradient_checkpointing:
        model.lang_model.enable_input_require_grads()
...
def load_huggingface_base_checkpoint(model, repo_id, local_files_only):
...
def save_compact_checkpoint(model, tokenizer, epoch, args):
    if args.rank != 0:
        return
    output_dir = Path(args.run_name) / "compact_checkpoints" / f"epoch_{epoch + 1}"
    model.lang_model.save_pretrained(output_dir / "phi3_lora_adapter")
    tokenizer.save_pretrained(output_dir / "tokenizer")
    torch.save(
        model.vision_tokenizer.state_dict(),
        output_dir / "vision_token_sampler.pt",
    )
...
def main():
    ...
    parser.add_argument("--pretrained_hf", type=str, default=None)
    ...
    parser.add_argument("--image_prefix", type=str, default="defect_dataset/")
    ...
    IMAGE_FOLDER_DICT_GCP[image_prefix] = str(Path(args.image_root).resolve()) + os.sep
    if args.rank == 0:
    ...
    if args.resume_from_checkpoint is None and not args.dryrun:
        if args.pretrained_hf is not None:
            load_huggingface_base_checkpoint(
                model,
                args.pretrained_hf,
                args.offline,
            )
            print("Finished loading Hugging Face base checkpoint...")
        elif args.pretrained is not None:
            assert os.path.exists(args.pretrained), "Pretrained checkpoint does not exist."
            _, _, checkpoint = load_checkpoint(args, model, pretrained=True)
            print("Finished loading checkpoint...")
        elif args.pretrained_vision_tokenizer is None:
            raise ValueError("Specify --pretrained_hf or --pretrained.")
    ...
    params_with_wd = [param for param in params_with_wd if param.requires_grad]
    params_without_wd = [param for param in params_without_wd if param.requires_grad]
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        save_compact_checkpoint(model, tokenizer, epoch, args)
